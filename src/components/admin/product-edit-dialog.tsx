...
import { generateProductDetails } from "@/lib/ai-dummy";

interface ProductEditDialogProps {
    isOpen: boolean;
    setIsOpen: (isOpen: boolean) => void;
    product: Product | null;
    onSave: (productData: Omit<Product, 'id'>, id: string, selectedFile: File | null) => void;
}

export function ProductEditDialog({ isOpen, setIsOpen, product, onSave }: ProductEditDialogProps) {
    ...
    const handleSubmit = async () => {
        ...
        const productData = {
            ...
            certifications: certificationsInput.split(',').map(tag => tag.trim()).filter(tag => tag),
            seoTitle,
            seoDescription,
            seoKeywords
        };
    ...
    const handleGenerateDetails = async () => {
        ...
        try {
            const result = await generateProductDetails({ name });
            if (result) {
                ...
                toast({ title: "AI Details Generated!", description: "Review the generated content before saving." });
            }
        } catch (error) {
            console.error("Failed to generate product details", error);
            toast({ title: "AI Generation Failed", description: "Could not generate product details. Please try again.", variant: "destructive" });
        } finally {
                ...
                <ScrollArea className="h-[70vh] pr-6">
                    <div className="space-y-6 py-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="varieties" value={varietiesInput} onChange={(e) => setVarietiesInput(e.target.value)} placeholder="e.g., 1121 Basmati, Pusa Basmati" />
                            <p className="text-xs text-muted-foreground">Enter tags separated by commas.</p>
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="certifications">Certifications</Label>
                            <Input id="certifications" value={certificationsInput} onChange={(e) => setCertificationsInput(e.target.value)} placeholder="e.g., Organic, Export Grade" />
                            <p className="text-xs text-muted-foreground">Enter tags separated by commas.</p>
                        </div>
                        ...
                        <div className="space-y-4 pt-4 border-t">
                            ...
                            <div className="grid grid-cols-1 items-center gap-4">
                                ...
                                <p className="col-span-4 text-xs text-muted-foreground">Enter keywords separated by commas.</p>
                            </div>
                        </div>
