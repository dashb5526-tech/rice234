...
export function GallerySectionEditDialog({ isOpen, setIsOpen, content, onSave }: GallerySectionEditDialogProps) {
    ...
    const handleAiGenerate = async (promptText: string) => {
        ...
        try {
            const result = await generateContent({ topic: "photo gallery of our process and products", prompt: promptText });
            if (result) {
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="gallery-title" value={currentContent.title} onChange={(e) => handleContentChange('title', e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="gallery-description">Description</Label>
                            <Textarea id="gallery-description" value={currentContent.description} onChange={(e) => handleContentChange('description', e.target.value)} />
                        </div>
                    </div>
                    <DialogFooter>
